...
class TaskDefinitionTest < ActiveSupport::TestCase
  ...
  context 'Single task validation' do
    ...
    test 'doesnt touch col1_id columns that are not references' do
      ...
      definition = ForemanDeployments::TaskDefinition.new @parent_task, {}, TestUserTask, @params
    ...
    test 'validates task output type' do
      ...
      TestUserTask.expects(:get_validation_object).returns('not validation result')

      assert_raise ArgumentError, 'validation object should inherit from ValidationResult' do
        definition.validate
      end
    end
  ...
  context 'Related tasks validation' do
    ...
    test 'changes col1_id columns to col1 for validation' do
      ...
      user_validation_result = ForemanDeployments::ValidationResult.new({ object_id: mock('user') }) do
        true
      end

      usergroup_validation_result = ForemanDeployments::ValidationResult.new({ object_id: mock('group') }) do
        true
      end

      TestUserTask.expects(:get_validation_object).returns(user_validation_result)
      TestUsergroupTask.expects(:get_validation_object).with do |actual_params|
        refute_nil actual_params[:params][:user] || actual_params[:params]['user']
        true
    ...
    test 'changes item_ids columns to items for validation' do
      ...
      TestUserTask.expects(:get_validation_object).returns(user_validation_result)
      TestUsergroupTask.expects(:get_validation_object).with do |actual_params|
        refute_nil actual_params[:params][:users].try(:first) || actual_params[:params]['users'].try(:first)
        true
      end.returns(usergroup_validation_result)
  ...
  test 'All tasks are planned' do
    ...
    end.times(5).returns(OpenStruct.new({
        output: { object_id: mock('myObj') }
      }))

    @definitions.values.each(&:plan)
  end
end
